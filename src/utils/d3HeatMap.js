...
export function heatmapDendro(data, parent) {

  if (!data || !data.matrix)
    return;
      

  var svg = d3.select(document).select('g#heatmap')
    .append('svg')
    .attr('width', '100%')
    .attr('height', '150px');
...
export function testt(document) {
  const g = d3.select(document).select('g#heatmap');
  g.append('circle')
